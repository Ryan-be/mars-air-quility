...
async function loadSparkline(inferenceId, inferenceAt) {
  ...
  try {
    ...
    const traces = data.triggering_channels.map(function (ch) {
      return {
        x: data.timestamps.map(ts => (new Date(ts).getTime() - inferenceTime) / 60000),
        y: data.channels[ch] || [],
        mode: 'lines', name: ch,
    ...
    const layout = {
      ...
      showlegend:false,
      shapes: [{ type:'line', x0:0,x1:0,y0:0,y1:1, xref:'x', yref:'paper', line:{color:'#ef4444',width:1.5,dash:'dash'} }],
      annotations: [{ x:0,y:1, xref:'x', yref:'paper', text:'Event', showarrow:false, font:{size:9,color:'#ef4444'}, yanchor:'bottom' }],
