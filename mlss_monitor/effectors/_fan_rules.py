"""Fan-family rule machinery (inlined from legacy ``fan_controller.py``).

This module holds the rule-based decision machinery the fan-family
effector controllers (:mod:`mlss_monitor.effectors.fan`) reuse, plus
the public :class:`FanController` aggregator and
:func:`build_default_controller` factory still consumed by
``mlss_monitor.app.log_data`` to populate the legacy
``/api/fan/auto-status`` snapshot.

History
-------
This was originally ``mlss_monitor/fan_controller.py``. Phase 12 of the
MLSS topology feature inlined it into the ``effectors`` package so the
package is self-contained and the legacy module can be deleted. The
leading underscore in the module name marks it as a package-private
helper — the public surface still flows through
:mod:`mlss_monitor.effectors.fan`.

Strategy pattern
----------------
Each :class:`FanRule` subclass encapsulates one decision criterion.
:class:`FanController` aggregates the registered rules and produces a
single fan action together with a human-readable explanation of *why*
each rule voted the way it did.

Adding a new rule is a two-step process:
    1. Create a subclass of :class:`FanRule`.
    2. Register an instance via :meth:`FanController.register`.
"""
