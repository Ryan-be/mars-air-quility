...
- ~~Custom Pi SD-card .img for one-step provisioning~~ — **infrastructure shipped:** `scripts/build_pi_image.sh` (wrapper around `pi-gen`), `scripts/stage-mlss-grow/` (apt package list, host-side + chroot run scripts, systemd unit, firstboot.sh, mlss-grow.yaml.template), `docs/PI_IMAGE_BUILD.md`. **Maintainer to-do** before first image release: run `bash scripts/build_pi_image.sh` on a Linux box (pi-gen needs chroot + binfmt_misc — won't work on macOS/Windows). The script depends on item #4 (`mlss-grow` on PyPI). After build, attach the resulting `dist/mlss-pi-os-<version>.img.xz` to a `pi-image-v0.1.0` GitHub Release.
