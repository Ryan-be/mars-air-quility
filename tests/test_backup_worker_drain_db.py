"""BackupWorker._drain_db_batch — drains outbox to Postgres.
...
Covered:
  - _parse_pk helper (single + composite + colons-in-text-pk)
  - empty outbox → no-op
...
@pytest.fixture
def worker():
    """Enabled BackupWorker — drain functions are no-ops in DISABLED but
    the state machine doesn't actually guard the drain entrypoint (the
    run loop in Task 15 does). Promoting to IDLE here just mirrors how
    the run loop will use it."""
...
def test_parse_pk_single_str():
    ...
    assert _parse_pk("INC-2026-05-18T12:00:00", [str]) == ("INC-2026-05-18T12:00:00",)
...
def test_parse_pk_composite_int_str():
    ...
    "3:pump" → (3, "pump")."""
    assert _parse_pk("3:pump", [int, str]) == (3, "pump")
...
def test_parse_pk_composite_str_int_with_colons_in_str():
    ...
    assert _parse_pk("INC-2026-05-18T12:00:00:5", [str, int]) == (
...
def test_parse_pk_composite_str_int_for_alerts():
    ...
    assert _parse_pk("INC-2026-05-18T12:00:00:42", [str, int]) == (
...
def test_drain_db_batch_empty_outbox_returns_false(db_path, worker, pg_client):
    ...
    with sqlite3.connect(db_path) as conn:
        result = worker._drain_db_batch(conn, pg_client)
    assert result is False
    pg_client.upsert_rows.assert_not_called()
    pg_client.delete_scope.assert_not_called()


def test_drain_db_batch_ships_one_table(db_path, worker, pg_client):
...
def test_drain_db_batch_groups_multiple_rows_per_table(db_path, worker, pg_client):
...
def test_drain_db_batch_splits_per_table(db_path, worker, pg_client):
...
def test_drain_db_batch_processes_delete_scope_before_rows(db_path, worker, pg_client):
...
def test_drain_db_batch_returns_true_when_only_delete_scope_processed(db_path, worker, pg_client):
...
def test_drain_db_batch_handles_composite_pk(db_path, worker, pg_client):
...
def test_drain_db_batch_missing_live_row_drops_outbox_entry(db_path, worker, pg_client):
...
def test_drain_db_batch_propagates_postgres_errors(db_path, worker, pg_client):
...
def test_drain_db_batch_propagates_delete_scope_errors(db_path, worker, pg_client):
