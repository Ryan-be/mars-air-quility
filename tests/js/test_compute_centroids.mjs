...
//   LANE_HEIGHT_PX(44) + stackSlots * STACK_DY_PX(26) + CYTO_HULL_PADDING_Y(30)
// where stackSlots = min(MAX_STACK_STEPS, max(1, ceil((primary-1)/2)))
// For primary=30: stackSlots=min(5, ceil(29/2))=5, halfH = 44 + 5*26 + 30 = 204
// For primary=2:  stackSlots=min(5, ceil(1/2))=1,  halfH = 44 + 1*26 + 30 = 100
const deep = [
...
// For deep rows half_h = 204: delta >= 408.
expect('deep rows: row1_y - row0_y >= 408 (2 * 204)',
  cd.D3.y - cd.D1.y >= 408, true);

// For shallow rows half_h = 100: delta >= 200.
expect('shallow rows: row1_y - row0_y >= 200 (2 * 100)',
  cs.S3.y - cs.S1.y >= 200, true);
...
expect('one-lane 10 primaries: row1_y - row0_y >= 408',
  cl.L3.y - cl.L1.y >= 408, true);

// --- Clamp boundary: single-primary incident --------------------------
// ceil((1-1)/2) = 0 → clamp to 1 slot → halfH = 44 + 1*26 + 30 = 100.
...
expect('singleton-primary clamp: row1_y - row0_y >= 200 (2 * 100)',
  cxl.X3.y - cxl.X1.y >= 200, true);
