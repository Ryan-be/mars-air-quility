"""BackupWorker._drain_files_batch — drains outbox_blobs to S3.
...
@pytest.fixture
def worker():
    """Enabled BackupWorker(pipeline='files'). The drain entrypoint
    isn't state-guarded (the run loop in Task 15 is) but we promote out
    of DISABLED to mirror how the run loop will call this."""
...
def test_drain_files_batch_empty_returns_false(db_path, worker, s3_client):
    """No pending blobs → no S3 calls, returns False. The run loop
    (Task 15) uses False to flip the worker state back to IDLE."""
    with sqlite3.connect(db_path) as conn:
        result = worker._drain_files_batch(conn, s3_client)
    assert result is False
    s3_client.put.assert_not_called()
    s3_client.head.assert_not_called()


def test_drain_files_batch_uploads_one_blob(db_path, worker, s3_client, tmp_jpeg):
...
def test_drain_files_batch_skips_put_when_head_returns_true(db_path, worker, s3_client, tmp_jpeg):
    ...
    with sqlite3.connect(db_path) as conn:
        worker._drain_files_batch(conn, s3_client)
    s3_client.put.assert_not_called()
    with sqlite3.connect(db_path) as conn:
        assert outbox.pending_count_blobs(conn) == 0


def test_drain_files_batch_drops_entry_when_source_missing(db_path, worker, s3_client):
...
def test_drain_files_batch_routes_by_target_key_prefix(db_path, worker, s3_client, tmp_path):
...
def test_drain_files_batch_unknown_target_key_logs_and_drops(db_path, worker, s3_client, tmp_path):
...
def test_drain_files_batch_propagates_s3_errors(db_path, worker, s3_client, tmp_jpeg):
...
def test_drain_files_batch_propagates_head_errors(db_path, worker, s3_client, tmp_jpeg):
...
def test_drain_files_batch_continues_after_drop_on_missing_source(db_path, worker, s3_client, tmp_jpeg):
