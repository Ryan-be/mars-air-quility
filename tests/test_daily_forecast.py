...
def _mock_daily(n=7):
    ...
    payload = {
        "daily": {
            ...
            "uv_index_max":                  [3.0  + i for i in range(n)],
            "sunrise":                       [f"2024-01-{i+1:02d}T07:{30+i:02d}" for i in range(n)],
            "sunset":                        [f"2024-01-{i+1:02d}T16:{30+i:02d}" for i in range(n)],
        }
    }
...
def test_day_has_required_keys(mock_urlopen, client):
    ...
    day = result["days"][0]
    for key in ("date", "temp_max", "temp_min", "precip_prob", "precip_sum",
                 "weather_code", "wind_speed", "uv_index", "sunrise", "sunset"):
